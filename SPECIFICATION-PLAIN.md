...
- **Reference implementation and test vectors:** the initial Rust implementation, five `PIM = 0`
  vectors covering every BIP39 source length, and one `PIM = 1` vector are available. Fast tests cover
...
Let `CS(E)` denote the 8-bit BIP39 checksum of 256-bit entropy `E`, and let `Perm_{P,PIM}`
be one fixed MHFE permutation.
...
```text
256 * 60 seconds = 15,360 seconds = 4 hours 16 minutes
```

The mean also hides a long tail. Under the same idealized independent-hit approximation, the number
of complete permutations `J` is geometrically distributed with success probability `rho = 1/256`:

```text
P(J > k) approximately (255/256)^k
```
...
geometric model with checksum-match probability `p_match = 1/256`, let the observed correct
...
```text
P(tau_prime = tau) = p_match / (2 - p_match) = 1/511
E[min(tau, tau_prime)] = 1 / (1 - (1-p_match)^2) approximately 128.25
```
...
Every vector uses the public ASCII password `public test password`; five use `PIM = 0` and the
additional 12-word vector uses `PIM = 1`. Each records the
...
derived from the specification rather than the Rust round code, reproduced all six vectors in both
...
- omitted PIM and explicit `PIM = 0` equivalence, additional non-zero PIM/source-length cases,
  machine-readable wrong-PIM recovery cases, and rejection of `PIM = 32`;
