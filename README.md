...
```math
256 \cdot 34.26\ \mathrm{s} = 8{,}770.56\ \mathrm{s} \approx 2\ \mathrm{h}\ 26\ \mathrm{min}
```
...
permutation applications and would restore the overall $`8 \cdot 256 = 2{,}048`$ expected-work
factor. A direct construction would instead require a new, separately analyzed 253-bit
permutation, new domain separation, new test vectors, and an explicit profile identifier.
